...
function PortalNav({ role }) {
  ...
  return (
    <header className="sr-portal-header sticky top-0 z-40 border-b border-[#dce8f4] bg-white/95 backdrop-blur">
      <div className="sr-portal-top mx-auto flex items-center justify-between gap-5 px-4 py-3 md:px-8">
        ...
        <div className="sr-portal-actions flex items-center gap-2"><span className="sr-live hidden sm:inline-flex">Live network</span><span className="sr-portal-language hidden md:inline-flex">◎ English⌄</span>{role === "patient" && <Link to="/patient/hospitals" className="sr-btn-primary">Find a hospital <span aria-hidden="true">↗</span></Link>}{role === "doctor" && <Link to="/doctor/referral" className="sr-btn-primary">New referral <span aria-hidden="true">+</span></Link>}<span className="sr-portal-avatar">{roleInitial}</span></div>
      </div>
      <nav className="sr-portal-nav mx-auto flex gap-1 overflow-x-auto border-t border-slate-100 px-4 py-2 md:px-8">{links.map(([text, path]) => <Link key={path} to={path} className={`sr-portal-link whitespace-nowrap rounded-xl px-3 py-2 text-xs font-extrabold ${location.pathname === path ? "active" : ""}`}><span aria-hidden="true">{icons[text] || "•"}</span>{text}</Link>)}</nav>
    </header>
  );
}
