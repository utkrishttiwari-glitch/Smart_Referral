...
const serviceImages = {
  emergency: "https://images.unsplash.com/photo-1584982751601-97dcc096659c?auto=format&fit=crop&w=900&q=80",
  trauma: "https://images.unsplash.com/photo-1559757175-0eb30cd8c063?auto=format&fit=crop&w=900&q=80",
  cardiac: "https://images.unsplash.com/photo-1530026405186-ed1f139313f8?auto=format&fit=crop&w=900&q=80",
};
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/hospitals/${candidate.id}/confirm`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ dataType: "BED_AVAILABILITY" }),
        }
      );
      const result = await response.json();
...
function ServiceCard({ service, selected, onSelect }) { const key = String(service.name || "").toLowerCase(); const image = key.includes("trauma") ? serviceImages.trauma : key.includes("cardio") ? serviceImages.cardiac : serviceImages.emergency; return <button type="button" onClick={onSelect} className={`overflow-hidden rounded-3xl border-2 bg-white text-left shadow-sm transition hover:-translate-y-1 hover:shadow-lg ${selected ? "border-blue-500 bg-blue-50/50 ring-2 ring-blue-100" : "border-transparent"}`}><div className="relative h-36 bg-blue-100"><img src={image} alt="" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.style.display = "none"; }} /><span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-blue-700">{selected ? "✓ Selected" : "Select"}</span></div><div className="p-5"><h3 className="text-lg font-bold text-slate-900">{service.name}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{service.description || "Specialist care matched to current hospital availability."}</p></div></button>; }
